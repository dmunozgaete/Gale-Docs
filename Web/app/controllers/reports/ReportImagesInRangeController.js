...
.controller('ReportImagesInRangeController', function (
	$scope, 
	$state, 
	$Api, 
	$karmaTable,
	...
	var _changeFilters = function(from,to,type){
		...
		_karmaComponent.setup('/Report/ImagesInRange?from={0}&to={1}&type={2}'.format([
	...
	$karmaTable.then(function(component){
		...
		component.$on("loadComplete", function(data){
			...
			_.each(data.items, function(item){
				...
				//FIX: KARMA-TABLE , not inherit $scope element's
	...
	$scope.export = function(){
		...
		var url = $Api.get_endpoint() + '/Printer/ImagesInRange?from={0}&to={1}&type={2}'.format([
